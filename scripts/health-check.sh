...
if ! lsof -i :8001 &>/dev/null; then
    echo -e "Port 8001 (Django)      : ${GREEN}✅ Available${NC}"
    SUCCESS_COUNT=$((SUCCESS_COUNT + 1))
else
    echo -e "Port 8001 (Django)      : ${YELLOW}⚠️  In use${NC}"
fi

if ! lsof -i :3001 &>/dev/null; then
    echo -e "Port 3001 (Frontend)    : ${GREEN}✅ Available${NC}"
    SUCCESS_COUNT=$((SUCCESS_COUNT + 1))
else
    echo -e "Port 3001 (Frontend)    : ${YELLOW}⚠️  In use${NC}"
fi
